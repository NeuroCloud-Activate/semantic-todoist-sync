...
8. **Opt-in Internet Search and Deep Research in chat.**
   The sidebar shows a compact labeled Internet Search modifier above the prompt; it is off for every new view and cycles `off → Internet Search → Deep Research → off` per request. Internet Search combines vault and web evidence in one paragraph; Deep Research uses the same bounded native web request, one local title-seeded retrieval pass, and two bounded provider generations (analysis then final answer) before returning two or three paragraphs. Cited external source titles become clickable links. Successful cited research is saved by default as `YYYY-MM-DD - Research Subject.md` under `Semantic Todoist Sync/Research`, with sanitized YAML frontmatter; the independent `Save Internet Search research` setting can turn that capture off.
