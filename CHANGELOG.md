...
- Updated AGENTS.md and requirements.md to name
  `Testing/[redacted-private-value]` as the live Obsidian testing vault and the
...
- Functional validation for this optimization uses the synchronized
  `Testing/[redacted-private-value]` vault, its Live Semantic Index, and normal
...
- Added an additive multi-provider operation map for OpenAI, Gemini, OpenRouter, and OpenWebUI, with explicit per-operation primary models, global fallback control, stable fallback validation, and searchable wide settings controls that preserve provider-scoped model IDs.
- Added task-local singleton description requests with stable-ID context projection, cache-aware shared prefixes, token-limit/preflight telemetry, and task-isolated evidence continuity through description and citation handling.
- Added provider-neutral context projection and fallback validation so selected evidence, citations, reasoning settings, and operation dispatch remain aligned across the four supported providers.
- Added automatic provider-native schema, reasoning, optional-parameter, caching, error, and usage mapping for each primary and fallback attempt, with schema-preserving compatibility retries and explicit failures for unsupported configured controls.
- Added provenance-aware primary and fallback reasoning per operation, including automatic capability-derived defaults, independent fallback settings, compatible model-switch migration, and full-width responsive controls.
- Added an OpenWebUI/Ollama grammar-compatible strict-schema projection that omits only unsupported regex patterns in transit and enforces the untouched patterns locally before normal workflow validation.
- Added model-affine OpenWebUI lanes for generation and embeddings: the plugin worker count defaults to 1, each model's same-model call limit defaults to 2 (max 4), and additional workers can serve distinct models; task descriptions follow the selected-model limit while model switches and embedding transitions remain sequential/FIFO. Context-window detection uses the exact active allocation from the selected model's proxied `/ollama/api/ps` first; `/api/models`, `/ollama/api/tags`, and `/ollama/api/show` remain informational metadata, then the manual model setting is the fallback, followed by the configurable unknown default of 16,384 tokens. Completed SSE-compatible responses use `stream: false`, and the manual editor remains one searchable model with Auto.
- Added a fair OpenRouter free-router profile that admits at most 20 calls per minute across generation, embeddings, and fallback attempts; other cloud providers default to 10 concurrent calls per provider/model.
- Added serialized, coalescing settings/state persistence so overlapping autosaves publish the newest complete revision, preserve unrelated fields, avoid partial state, and recover after rejected writes.
- Kept semantic-index, task-reference, scheduler-memory, and routing-artifact persistence on the shared coordinator boundary while preserving atomic publication and last-good recovery.
- Refreshed responsive searchable model controls and current provider/privacy documentation for the four-provider workflow.
- Added task- and scope-relevant stable-ID deduplication before every context-consuming provider input, plus one adaptive context-window safety curve from 80% at or below 4,096 tokens to 50% at 100,000 tokens and above; unknown windows use an explicit 16,384-token operational ceiling across chat, task generation, descriptions, deduplication, scheduling, policy, and section naming. Full-envelope preflight includes the strict schema and stable prefix with a conservative safety margin, while task, description, and AI section-title calls receive request-derived native output budgets.
- Kept selected chat evidence and history complete: the plugin never token-truncates or drops them, and oversized complete requests fail observably before dispatch. OpenWebUI timeout callers now leave non-abortable transports draining in FIFO order, while usage and diagnostics report the actual attempted provider/model.
- Added one shared 180-second gateway attempt deadline across every provider and operation kind, including generation, embeddings, discovery, and setup checks. Non-abortable timed-out attempts drain before the lane can settle; late results, usage, and state changes are suppressed, and fallback cannot overlap the draining attempt.
- Added a passive live-validation watchdog so diagnostic harnesses cannot remain unboundedly hung without changing provider retry or fallback behavior.
- Kept `Refresh` and `Test AI` action rows at readable intrinsic widths that stack full-width on narrow panes, and expanded the Selected AI configuration summary to Chat, Task generation, Task descriptions, and Embeddings while showing fallback columns only when enabled.
